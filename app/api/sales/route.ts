...
export async function GET(request: Request) {
  ...
  const salesData = await sql(
    `SELECT sale_date, channel, SUM(sale_amount) AS total_sales FROM sales WHERE sale_date BETWEEN $1 AND $2 GROUP BY sale_date, channel ORDER BY sale_date, channel;`,
    [startDate, endDate],
  )
